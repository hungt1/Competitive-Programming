...
void add(int val){
    s2.insert(val); sumRight += val;
    while (s2.size() && (s1.size() < mid)){
        auto it = s2.begin();
        sumLeft += *it; sumRight -= *it;
...
int main()
{
    ...
    cout << 1ll * (*s1.rbegin()) * mid - sumLeft + sumRight - 1ll * (k - mid) * (*s1.rbegin()); 
    for (int i = k; i < n; i++){
        add(a[i]);
        remove(a[i - k]);
        int val = *s1.rbegin();
        cout << ' ' << 1ll * val * mid - sumLeft + sumRight - 1ll * (k - mid) * val;
    }
    return 0;
}